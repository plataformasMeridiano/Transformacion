"""
run_puente_backfill.py — Descarga boletos Puente desde ene-15 a hoy.
"""
import asyncio
import json
import logging
...
async def main() -> int:
    ...
    tag = f"puente_backfill_{inicio.isoformat()}_a_{fin.isoformat()}"
    setup_logging(general["log_dir"], tag)
    logger = logging.getLogger("puente_backfill")
    logger.info("=" * 60)
    logger.info("Puente backfill — %s a %s (%d días hábiles)", inicio, fin, len(fechas))
