"""
run_win_zapier.py — Dispara Zapier solo para WIN en las fechas del backfill feb–mar 2026.
"""
import json
import logging
import os
...
def get_fechas() -> list[str]:
    base = Path(__file__).parent / "downloads" / "WIN"
    fechas = sorted(
        d.name for d in base.iterdir()
        if d.is_dir()
        and any(d.rglob("*.pdf"))
        and (d.name.startswith("2026-02") or d.name.startswith("2026-03"))
    )
    return list(reversed(fechas))  # más reciente primero
...
def wait_done(fecha: str) -> bool:
    ...
    while elapsed < MAX_WAIT:
        ...
        logging.info("[%s] %ds — registros WIN: %d", fecha, elapsed, len(records))
        for rec in records:
...
def main():
    fechas = get_fechas()
    logging.info("Fechas WIN a procesar: %d", len(fechas))
    for f in fechas:
        logging.info("  %s", f)
