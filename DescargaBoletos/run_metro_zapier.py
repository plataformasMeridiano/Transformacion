"""
run_metro_zapier.py — Dispara Zapier solo para MetroCorp en las fechas con boletos descargados.
"""
import json
import logging
import os
...
def get_fechas(desde: str = "2026-02-01") -> list[str]:
    base = Path(__file__).parent / "downloads" / "MetroCorp"
    fechas = sorted(
        d.name for d in base.iterdir()
        if d.is_dir() and d.name >= desde and any(d.rglob("*.pdf"))
    )
    return fechas
...
def wait_done(fecha: str) -> bool:
    ...
    while elapsed < MAX_WAIT:
        ...
        logging.info("[%s] %ds — registros MetroCorp: %d", fecha, elapsed, len(records))
        for rec in records:
...
def main():
    fechas = get_fechas("2026-02-01")
    logging.info("Fechas con boletos MetroCorp: %d", len(fechas))
    for f in fechas:
        logging.info("  %s", f)
