...
| ConoSur | sistemaD | `alyc_sistemaD.py` | Dos instancias: cuenta 3003 (MN) y 3087 (Pamat). API vía **proxy del portal** (no `vb-back`), autentica por cookie — ver nota abajo |
...
- **IEB — descarga por fecha de CONCERTACIÓN (rediseño 2026-07-14, commit `2d32cfb`):** el enfoque original archivaba las cauciones filtrando `CuentaCorrientePesos` (proceso=02) por `FEC1`, que para la pata de cierre (`TOCT`) es la **fecha de liquidación** → el cierre quedaba bajo su vencimiento y no bajo la concertación, y se perdían cierres de plazo largo. Nuevo flujo en dos pasos:
  ...
  - **Backfill junio 2026 rehecho:** se mandaron a papelera los 15 boletos viejos mal fechados y se re-descargaron **20 boletos** correctos por concertación (17–30/jun). Incluye cierres de plazo largo antes perdidos (ej. caución 23/06 con cierre `807620` que liquida 23/07). **`pase_codes` = `["CMAE","VMAE"]`** (commit `029c523`, 2026-09-03): CMAE = compra MAE contado inmediato, VMAE = venta MAE 24 horas. Estaban sin configurar y el descarte se loguea en DEBUG, así que se perdieron **17 pases de agosto (34 boletos)** en silencio — recuperados. Ojo: son códigos genéricos de compra/venta en MAE, así que una operación suelta de títulos ahí quedaría archivada como pase (los títulos outright usan `CRCN`). **El Zap no tiene rama de pases para IEB**: esos boletos se descargan pero no generan issue solos. `colocadoras_codes` aún por descubrir; Títulos IEB soportado pero **inactivo** (falta agregar `"Títulos"` a `tipo_operacion` + `titulos_codes`).
...
- **ConoSur — migró el login a Auth.js (2026-09-18, commit `587ba14`):** `/api/auth/session` dejó de devolver `accessToken` (hoy trae solo `user`/`expires`/`session_state`/`roles`), así que el scraper moría con **`KeyError: 'accessToken'`** en cada fecha — el login seguía reportando "exitoso" porque solo se chequea la URL. El token pasó a vivir cifrado en la cookie de sesión y lo agrega el Next.js del lado servidor: **pegarle a `vb-back-conosur.aunesa.com` con las cookies da 401, y al mismo path bajo `virtualbroker-conosur.aunesa.com` da 200.** Se movió `_API_BASE` al host del portal y se eliminó el paso del JWT. `page=1` se deja: la API trata 0 y 1 como la primera página.
  **Ojo — el KeyError es reciente pero la rotura no:** ConoSur venía informando `desc=0 err=0` desde mediados de junio (cero mudo, tres meses) y el error apareció recién en septiembre. No se perdió ningún boleto porque **ConoSur dejó de operar cauciones**: la 3003 no tiene ninguna desde mayo y la 3087 cerró el 03/06 con 6 tomadoras, que son exactamente los 6 boletos que ya estaban en Supabase. Las cuentas siguen vivas (posiciones de $16,8M y $5M, con extracciones en agosto).

- **Allaria — el entry point del SSO cambió (2026-08-20, commit `1f9003d`):** `https://allaria.com.ar/Account/RedirectLogin` (el del portal viejo) empezó a devolver **404** y a renderizar la home institucional, así que la página nunca redirigía y el login moría esperando. Se entra por **`app.allaria.com.ar`**, que redirige solo a `login.allaria.com.ar/u/login`; se toma del `url_login` del config. Estuvo 6 días caído (15→20/08, alerta de Slack) y se perdieron 2 boletos, recuperados con backfill.
  **Lección que se repitió:** los chequeos de URL por substring son una trampa acá. El entry point es `/Account/Redirect**L**ogin` — preguntar `"login" in url` daba **falso** por la mayúscula (y en minúsculas habría dado **verdadero** sobre el propio entry point). Ahora se decide con `_en_login()` / `_en_app()`, que normalizan y apuntan al **destino positivo** (la app) en vez de a la negación de "login".
...
**Una sola corrida por día desde el 18/09/2026, por costo de Zapier.** Antes corría también a las 18:00. La fase 5 dispara el webhook para los últimos `VENTANA_DIAS = 5` días hábiles **sin chequear si la fecha ya está procesada** (`process_fecha` consulta `is_done` recién después de disparar), así que cada fecha de operación se procesaba ~6 veces: medido sobre `Procesamiento_Cauciones`, una pasada cuesta ~38 filas y las fechas del medio de septiembre acumulaban ~225. Con una corrida queda en ~3. **Además `run_boletos_zapier.py` ahora dispara solo si hay diferencia real** (commit `03edc1f`): algún boleto en `procesamiento_boletos` con `jira_issue_key` en null. Se excluyen los tipos que este webhook no procesa (`Títulos`, `Venta FCE-eCheq`) y los boletos con `reproceso_intentos >= 3`, porque si no una fecha con algo irrecuperable dispararía para siempre — le pasaría a los 2 Títulos de Puente del 14/09 y a los pases de IEB. Ante un error de consulta dispara todo, y `--forzar` saltea el chequeo para backfills. `VENTANA_DIAS` bajó de 5 a 3 en el mismo movimiento.

Con eso una fecha ya resuelta cuesta **0 tasks**. La mitad de las filas del relevamiento (831 de 1.689 en 10 días) eran `Ok - Sin archivos`, porque el disparo va sin filtro de ALyC y el Zap recorre las 13 aunque no tengan nada.
