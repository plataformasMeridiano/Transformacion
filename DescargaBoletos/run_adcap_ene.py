"""
run_adcap_ene.py — Descarga ADCAP para ene 15-30 2026 usando el filtro de fecha.
"""
import asyncio
import json
import sys
...
async def main() -> int:
    ...
    inicio = date(2026, 1, 26)
    fin    = date(2026, 1, 30)
    fechas = business_days(inicio, fin)

    import logging
    tag = f"adcap_{inicio.isoformat()}_a_{fin.isoformat()}"
    setup_logging(general["log_dir"], tag)
    logger = logging.getLogger("adcap_batch")
    logger.info("=" * 60)
    logger.info("ADCAP batch — %s a %s (%d días hábiles)", inicio, fin, len(fechas))
    ...
    alycs = [a for a in config["alycs"] if a.get("activo") and a["nombre"] == "ADCAP"]
    logger.info("Entradas ADCAP: %d", len(alycs))
